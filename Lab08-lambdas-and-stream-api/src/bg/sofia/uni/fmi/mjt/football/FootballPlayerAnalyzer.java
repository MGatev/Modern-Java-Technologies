...
public class FootballPlayerAnalyzer {
    private static final int MAX_DIFFER = 3;
    ...
    public FootballPlayerAnalyzer(Reader reader) {
        try (var input = new BufferedReader(reader)) {
            players = input.lines().skip(1).map(Player::of).toList();
        } catch (IOException e) {
            throw new UncheckedIOException("There is a problem with the file", e);
        }
    }
    ...
    public Player getHighestPaidPlayerByNationality(String nationality) {
        ...
        long maxWage =  players.stream().mapToLong(Player::wageEuro).max().orElse(-1);
    ...
    public Map<Position, Set<Player>> groupByPosition() {
        Map<Position, Set<Player>> groupMap = new HashMap<>();
        for (Position pos : Position.values()) {
            Set<Player> validPlayers = players.stream()
                .filter(player -> player.positions().contains(pos))
                .collect(Collectors.toSet());

            if (!validPlayers.isEmpty()) {
                groupMap.put(pos, validPlayers);
            }
        }
        return groupMap;
    }
    ...
    public Set<Player> getSimilarPlayers(Player player) {
        ...
        Set<Player> returned = players.stream()
            .filter(player1 -> player1.positions().stream().anyMatch(player.positions()::contains))
            .filter(player1 -> player1.preferredFoot().equals(player.preferredFoot()))
            .filter(player1 -> player1.overallRating() <= player.overallRating() + MAX_DIFFER &&
                player.overallRating() <= player1.overallRating() + MAX_DIFFER).collect(Collectors.toSet());
